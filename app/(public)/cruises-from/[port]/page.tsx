...
import {
  getPortById, searchSailings, getDestinations,
...
export default async function CruisesFromPortPage({ params, searchParams }: Props) {
  const { port: portId } = await params;
  const port = getPortById(portId);
  if (!port) notFound();
  ...
  const filters: SearchFilters = { departurePort: portId };
